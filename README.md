...
#### 1，arthas功能更全面，代码无侵入性，但是在耗时调优跟业务逻辑debug这块，缺点是粒度过粗（方法）或过细（每一行代码），对参数、局部变量、结果收集体验不好，其次是只能在线统计较短时间的请求，另外debug用户请求支持不够好，真正最需要的是进程内一个完整接口（底层函数）的完整片段逻辑的链路，而arthas只做到了单个方法级别，基本定位不了问题。arthas更适合处理极端性能或极端问题，对于通用的耗时和业务逻辑debug不太应手。
...
#### 4，个人觉得arthas的工具做的很多很全很厉害，但是未能针对特定几大类问题做对应的完整的解决方案，是功能而不是工具利器
